...
export function SingleCalendar() {
  ...
  // Current Month ID
  const currentMonthId = toDateId(new Date());

  return (
    <View className="flex-1 px-2 pt-2 bg-card h-fit">
      <Month 
        item={{ id: currentMonthId }} 
        stats={stats} 
        budget={budget} 
...
const Month = ({ item, stats, budget, selectedDate, setSelectedDate }: any) => {
    ...
    return (
        <View className="pb-2 px-2 bg-card h-fit">
...
const Day = ({ metadata, stats, budget, selectedDate, onPress }: any) => {
  ...
  if (isLogged) {
    if (percentage > 100) {
      backgroundColor = colorScheme === 'dark' ? '#C084FC' : "#a855f7"; // purple-500
      textColor = colorScheme === 'dark' ? "black" : "white";
    } else if (percentage > 50) {
      backgroundColor = colorScheme === 'dark' ? '#F87171' : "#ef4444"; // red-500
      textColor = colorScheme === 'dark' ? "black" : "white";
    } else if (percentage > 25) {
      backgroundColor = colorScheme === 'dark' ? '#FACC15' : "#eab308"; // yellow-500
      textColor = colorScheme === 'dark' ? "black" : "white"; 
    } else {
      backgroundColor = colorScheme === 'dark' ? '#4ADE80' : "#22c55e"; // green-500
      textColor = colorScheme === 'dark' ? "black" : "white";
    }
  }
  ...
  return (
    ...
    >
      <View 
        ...
        >
        <Text 
          className={isDifferentMonth ? "text-muted-foreground" : (isLogged ? "" : "text-foreground")}
        ...
        {isLogged && (
          <Text className="text-foreground" style={{ fontSize: 10, color: textColor, fontWeight: "bold", position: 'absolute', bottom: 3 }}>
            {formatTime(minutes)}
          </Text>
        )}
      </View>
